...
from model.green_swin_models import SwinTransformer, window_partition_and_flatten
...
class MaskedAutoencoder(nn.Module):
    ...
    def __init__(self, encoder, embed_dim, patch_size, in_chans=3, decoder_embed_dim=512,
        ...
        decoder_num_patches = self.encoder.patch_embed.num_patches
    ...
    def patchify_raw(self, imgs, patch_size=None):
        """
        imgs: (N, 3, H, W)
        our imgs: (N, 1, H, W)
        x: (N, L, patch_size**2 *3)
        our x: (N, L, patch_size**2 *1)
        """
    ...
    def patchify(self, imgs, patch_size=None):
        ...
        is_xuliehua = False
        if is_xuliehua:
            return self.patchify_ycc(imgs, patch_size)
        else:
            return self.patchify_raw(imgs, patch_size)
    ...
    def random_masking_raw(self, x, mask_ratio):
        """
    ...
    def random_masking(self, x, mask_ratio):
        """
        对每个样本独立进行随机掩码（per-sample shuffling）。
        x: [B, L, D]
        """
        N, L = x.shape[0], self.num_patches  # batch, length
        len_keep = int(L * (1 - mask_ratio))

        noise = torch.rand(N, L, device=x.device)  # [B, L]
...
def MAE_BTRFormer(img_size, img_patch_size, window_size, **kwargs):

...
from model.group_window_attention import WindowAttention, GroupingModule, get_coordinates
...
class BaseGreenModel(nn.Module):
    ...
    def apply_mask(self, x, mask, patches_resolution):
        ...
        coords = coords.reshape(1, H * W, 2).expand(B, -1, -1)  # [B, N, 2]
        ...
        coords_vis = []
        for i in range(B):
            x_vis.append(x[i][vis_mask[i]])
            coords_vis.append(coords[i][vis_mask[i]])
        x_vis = torch.stack([v for v in x_vis], dim=0)  # [B, N_vis, C]
        coords_vis = torch.stack([v for v in coords_vis], dim=0)  # [B, N_vis, 2]

        return x_vis, coords_vis, vis_mask
...
# raw
class PatchEmbed_raw(nn.Module):
...
def window_partition_and_flatten(x, window_size):
    """
    x: [B, dim, Ph, Pw]
    window_size: int
    返回: [B, dim, num_windows, window_size*window_size]
    """
    B, dim, Ph, Pw = x.shape
    assert Ph % window_size == 0 and Pw % window_size == 0, "Ph和Pw必须能被window_size整除"
    packets = []
    for i in [0, Ph // 2]:
        for j in [0, Pw // 2]:
            packets.append(x[:, :, i:i + Ph // 2, j:j + Pw // 2])  # [B, dim, Ph//2, Pw//2]

    patches = []
    for packet in packets:
        H, W = packet.shape[2], packet.shape[3]
        packet = packet.unfold(2, 2, 2).unfold(3, 2, 2)  # [B, dim, H//2, W//2, 2, 2]
        packet = packet.contiguous().view(B, dim, -1, 2, 2)  # [B, dim, num_patches, 2, 2]
        packet = packet.view(B, dim, -1, 4)  # [B, dim, num_patches, 4]
        patches.append(packet)

    out = torch.cat(patches, dim=2)
    out = out.reshape(B, dim, -1)
    return out


class PatchEmbed_ycc(nn.Module):
    r""" Image to Patch Embedding
    ...
    def forward(self, x):
        ...
        assert H == self.img_size[0] and W == self.img_size[1], \
            f"Input image size ({H}*{W}) doesn't match model ({self.img_size[0]}*{self.img_size[1]})."
        x = self.proj(x)  # torch.Size([B, dim, Ph, Pw])
        x = window_partition_and_flatten(x,
                                         window_size=2)  # [B, dim, num_windows_h * num_windows_w, window_size * window_size]
        x = x.transpose(1, 2)
...
class SwinTransformer(BaseGreenModel):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        is_xuliehua = False
        if is_xuliehua:
            self.patch_embed = PatchEmbed_ycc(
                img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim,
                norm_layer=norm_layer if self.patch_norm else None)
        else:
            self.patch_embed = PatchEmbed_raw(
                img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim,
                norm_layer=norm_layer if self.patch_norm else None)
        ...
        if self.ape:
            ...
            # self.pos_embed = self.absolute_pos_embed
